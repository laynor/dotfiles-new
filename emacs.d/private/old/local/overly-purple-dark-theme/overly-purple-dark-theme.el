;;; overly-purple-dark-theme.el --- Color theme with a dark and light versions.

;; Copyright (C) 2015 , Alessandro Piras

;; Author: Alessandro Piras
;; URL: FIXME: <https://github.com/nashamri/spacemacs-theme>
;;
;; Version: 0.1
;; Keywords: color, theme
;; Package-Requires: ((emacs "24"))

;; This theme has been created using the spacemacs theme as a base.
...
;; This is a color theme for spacemacs FIXME: <https://github.com/syl20bnr/spacemacs>.
;; It comes with two versions, dark and light and should work well in
;; a 256 color terminal.

;;; Code:

(defgroup overly-purple-theme nil
  "Overly-Purple-theme options."
  :group 'faces)

(defcustom overly-purple-theme-comment-bg t
  "Use a background for comment lines."
  :type 'boolean
  :group 'overly-purple-theme)

(defcustom overly-purple-theme-org-height t
  "Use varying text heights for org headings."
  :type 'boolean
  :group 'overly-purple-theme)

(defcustom overly-purple-theme-org-highlight nil
  "Highlight org headings."
  :type 'boolean
  :group 'overly-purple-theme)

(defun create-overly-purple-theme (variant theme-name)
  (let ((class '((class color) (min-colors 89))) ;;                    ~~ Dark ~~                                   ~~ Light ~~
        ...
        (bg1           (if (eq variant 'dark) (if (display-graphic-p) "#03000a" "#070020") (if (display-graphic-p) "#fbf8ef" "#ffffff")))
        (bg2           (if (eq variant 'dark) (if (display-graphic-p) "#10001a" "#1c1c1c") (if (display-graphic-p) "#efeae9" "#e4e4e4")))
        ...
        (key1          (if (eq variant 'dark) (if (display-graphic-p) "royal blue" "royal blue") (if (display-graphic-p) "#4f97d7" "#4f97d7")))
        (key2          (if (eq variant 'dark) (if (display-graphic-p) "deep sky blue" "deep sky blue") (if (display-graphic-p) "#277bb8" "#277bb8")))
        (builtin       (if (eq variant 'dark) (if (display-graphic-p) "medium slate blue" "#268bd2") (if (display-graphic-p) "#1f71ab" "#268bd2")))
        (keyword       (if (eq variant 'dark) (if (display-graphic-p) "dodger blue" "dodger blue") (if (display-graphic-p) "#237fbf" "#268bd2")))
        (const         (if (eq variant 'dark) (if (display-graphic-p) "light slate blue" "light slate blue") (if (display-graphic-p) "#4e3163" "#8700af")))
        (comment       (if (eq variant 'dark) (if (display-graphic-p) "MediumPurple" "MediumPurple") (if (display-graphic-p) "#2aa1ae" "#008787")))
        (comment-del   (if (eq variant 'dark) (if (display-graphic-p) "MediumPurple1")))
        (comment-bg    (if (eq variant 'dark) (if (display-graphic-p) "#080017" "#090026") (if (display-graphic-p) "#ecf3ec" "#ffffff")))
        (func          (if (eq variant 'dark) (if (display-graphic-p) "deep pink" "deep pink") (if (display-graphic-p) "#6c3163" "#8700af")))
        (str           (if (eq variant 'dark) (if (display-graphic-p) "blue violet" "blue violet") (if (display-graphic-p) "#2d9574" "#2aa198")))
        (type          (if (eq variant 'dark) (if (display-graphic-p) "spring green" "spring green") (if (display-graphic-p) "#ba2f59" "#af005f")))
        (comp          (if (eq variant 'dark) (if (display-graphic-p) "#c56ec3" "#d75fd7") (if (display-graphic-p) "#6c4173" "#8700af")))
        (var           (if (eq variant 'dark) (if (display-graphic-p) "forest green" "forest green") (if (display-graphic-p) "#715ab1" "#af5fd7")))
...
;;;;; basics
     ...
     `(font-lock-comment-face ((,class (:foreground ,comment :background ,(when overly-purple-theme-comment-bg comment-bg)))))
     `(font-lock-comment-delimiter-face ((,class (:foreground ,comment-del))))
...
;;;;; org
     ...
     `(org-agenda-date ((,class (:foreground ,var :height ,(if overly-purple-theme-org-height 1.1 1.0)))))
     `(org-agenda-date-today ((,class (:foreground ,keyword :slant italic :weight bold :height ,(if overly-purple-theme-org-height 1.3 1.0)))))
     `(org-agenda-date-weekend ((,class (:weight bold :foreground ,var))))
     `(org-agenda-done ((,class (:foreground ,(if (eq variant 'dark) suc green) :height ,(if overly-purple-theme-org-height 1.2 1.0)))))
     ...
     `(org-document-title ((,class (:foreground ,func :weight bold :height ,(if overly-purple-theme-org-height 1.4 1.0) :underline t))))
     `(org-done ((,class (:foreground ,(if (eq variant 'dark) suc green) :bold t :background ,org-h3-bg))))
     `(org-ellipsis ((,class (:foreground ,builtin))))
     `(org-footnote  ((,class (:underline t :foreground ,base))))
     `(org-hide ((,class (:foreground ,base))))
     `(org-kbd ((,class (:inherit region :foreground ,base :box (:line-width 1 :style released-button)))))
     `(org-level-1 ((,class (:bold t :foreground ,inf :height ,(if overly-purple-theme-org-height 1.3 1.0) :background ,(when overly-purple-theme-org-highlight org-h1-bg)))))
     `(org-level-2 ((,class (:bold t :foreground ,str :height ,(if overly-purple-theme-org-height 1.2 1.0) :background ,(when overly-purple-theme-org-highlight org-h2-bg)))))
     `(org-level-3 ((,class (:bold nil :foreground ,green :height ,(if overly-purple-theme-org-height 1.1 1.0) :background ,(when overly-purple-theme-org-highlight org-h3-bg)))))
     `(org-level-4 ((,class (:bold nil :foreground ,yellow :background ,(when overly-purple-theme-org-highlight org-h4-bg)))))
     ...
     `(org-scheduled-today ((,class (:foreground ,func :height ,(if overly-purple-theme-org-height 1.2 1.0)))))
...
(deftheme overly-purple-dark "Overly purple theme, the dark version")

(create-overly-purple-theme 'dark 'overly-purple-dark)

(provide-theme 'overly-purple-dark)

;;; overly-purple-dark-theme.el ends here
