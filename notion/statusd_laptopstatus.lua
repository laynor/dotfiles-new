...
if not statusd_laptopstatus then
  statusd_laptopstatus = {
    ...
    bat_now_sysfs_plugged = {"/sys/class/power_supply/BAT0/charge_now"},
    bat_full_sysfs_plugged = {"/sys/class/power_supply/BAT0/charge_full"},
    bat_now_sysfs_unplugged = {"/sys/class/power_supply/BAT0/energy_now"},
    bat_full_sysfs_unplugged = {"/sys/class/power_supply/BAT0/energy_full"}
  }
end
...
local function get_battery_sysfs()
    ...
    if pcall(function ()
        local file=try_open(statusd_laptopstatus.bat_now_sysfs_plugged)
        if file == nil then
           file = try_open(statusd_laptopstatus.bat_now_sysfs_unplugged)
        end
        now = tonumber(file:read("*all"))
        file:close();

        local file=try_open(statusd_laptopstatus.bat_full_sysfs_plugged)
        if file == nil then
           file = try_open(statusd_laptopstatus.bat_full_sysfs_unplugged)
        end
